...
class SemanticMatcher:
    ...
    def analyze(self,articles:Iterable[dict]):
        ...
        for a,(start,end) in zip(articles,spans):
            ...
            doc=av.mean(axis=0);n=float(np.linalg.norm(doc));doc=doc/n if n else doc;sims=np.max(av@self.matrix.T,axis=0);rm,rmean=self._agg(sims,'rule');km,kmean=self._agg(sims,'notion');em,emean=self._agg(sims,'private')
            out[str(a.get('id') or '')]=SemanticResult(quantize(doc.astype(np.float32)),rm,km,em,rmean,kmean,emean)
        return out
...
def semantic_adjustment(text,result,*,quality,contradiction,practical):
    ...
    eb=.32 if quality and rule>=.75 else (.18 if quality and fit>=.73 else 0);bb=.48 if contradiction and rule>=.74 else (.28 if contradiction and fit>=.72 else 0);pb=.24 if practical and fit>=.75 else 0;rp=0
    if know>=.86 and not quality and not contradiction and gb==0:rp=min(.8,.22+(know-.86)*4)
    elif know>=.9 and quality and rule<.76:rp=min(.35,.12+(know-.9)*2)
    bonus=max(-.9,min(1.65,fb+gb+eb+bb+pb-rp));labels=[]
    ...
    d={**result.public_dict(),'fit_bonus':round(fb,3),'gap_bonus':round(gb,3),'evidence_bonus':round(eb,3),'boundary_bonus':round(bb,3),'practical_bonus':round(pb,3),'repetition_penalty':round(rp,3),'semantic_bonus':round(bonus,3)}
