...
KEYWORDS={
 ...
 'marketing':['マーケティング','ブランド','広告','CRM','SEO','検索','コンテンツ','販促'],
...
def heuristic(src,title,summary,content=''):
    ts=tags(' '.join([title,summary,content[:5000]])); score=5+PRIORITY.get(src.get('priority','medium'),0)
    ...
    return score,notion,ts,'；'.join(parts) if parts else '与核心主题相关度有限，主要作为信息雷达保留'
...
def llm_screen(a):
    ...
    try:
        ...
        schema={'type':'object','properties':{'grade':{'type':'string','enum':['S','A','B','C']},'reading_score':{'type':'number','minimum':0,'maximum':10},'notion_score':{'type':'number','minimum':0,'maximum':10},'reason':{'type':'string'},'tags':{'type':'array','items':{'type':'string'},'maxItems':6}},'required':['grade','reading_score','notion_score','reason','tags'],'additionalProperties':False}
        prompt=f'''你替一名长期从事日本EC/Marketing、目前关注日本GTM、消费者行为、AI实际应用的人筛选每周文章。核心标准：认知增量、决策价值、实证/原始数据、可复用框架、日本市场/GTM/消费者/营销/EC/AI工作流相关。降低泛趋势、基础知识、纯PR、重复新闻、普通新品信息。S=必看，A=值得看，B=摘要即可，C=跳过。来源：{a['source']} 标题：{a['title']} 摘要：{a.get('summary','')} 正文可见片段：{a.get('content_excerpt','')} 仅根据可见信息判断。'''
...
def main():
    ...
    arts=list(existing.values());arts.sort(key=lambda a:(a.get('published') or a.get('first_seen') or ''),reverse=True)
