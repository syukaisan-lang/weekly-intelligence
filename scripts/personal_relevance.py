...
def item_text(x):
    ...
    return ' '.join(str(x.get(k,'') or '') for k in ('title','summary','page_body','text','content','memo','rule','core','when'))+' '+comments
...
def build_profile():
    ...
    return {'work':work,'knowledge':know,'weights':weighted,'knowledge_counts':know_counts,'available':bool(work or know)}
...
def score_text(text:str):
    ...
    bonus=max(-.9,min(1.8,fit_bonus+gap_bonus+evidence_bonus+boundary_bonus+practical_bonus-repetition_penalty))
    labels=[]
    if boundary_bonus:labels.append('可能修正现有规则/补充边界')
    if gap_bonus:labels.append('补足当前知识空白')
