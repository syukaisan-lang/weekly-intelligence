...
(() => {
  ...
  function restoreLegacyPositiveAutoRead(id){
    const cur=state?.[id];if(!cur||!POSITIVE.has(cur.feedback)||cur.status!=='read')return false;
    // Only repair records provably created by the old automatic-feedback migration.
    // Never touch genuine manual read marks or Later bookmarks.
    if(!['feedback','positive_feedback_only'].includes(cur.status_action))return false;
    const now=Date.now();cur.status='new';cur.status_action='positive_feedback_only';cur.status_updated_at=now;cur.updated_at=now;state[id]=cur;return true;
  }
  let migrated=false;
  for(const id of Object.keys(state||{})){migrated=restoreLegacyPositiveAutoRead(id)||migrated;migrated=markNegativeProcessed(id)||migrated;}
