// Weekly v9.1: local-only source audit + clean unread queue + implicit source passes.
...
(() => {
  ...
  function sourceStats(){
    ...
    for(const a of articleRows()){
      ...
      if(!map.has(name))map.set(name,{source:name,total:0,explicitSkip:0,implicitSkip:0,negative:0,positive:0,sa:0,saAdopted:0,saIgnored:0,bc:0,read:0,save:0,unread:0,expired:0});
  ...
  function ensureUi(){
    ...
    card.innerHTML=`<div class="section-head slim"><div><h2>来源质量</h2><div id="sourceAuditSummary" class="muted small">读取本机标记…</div></div><div id="sourceAuditPill" class="pill">0 停用候选</div></div>
      <p class="muted small">来源评估中，“未标注”按隐性跳过计算；但不会写成你的人工跳过，也不会作为强负反馈训练内容偏好。S/A 7天未处理会移出未读推荐。</p>
  ...
  function renderSourceAudit(){
    ...
    const totals=rows.reduce((o,r)=>{o.total+=r.total;o.skip+=r.skip;o.explicit+=r.explicitSkip;o.implicit+=r.implicitSkip;o.bc+=r.bc;o.sa+=r.sa;o.saAdopted+=r.saAdopted;o.save+=r.save;o.unread+=r.unread;o.expired+=r.expired;return o;},{total:0,skip:0,explicit:0,implicit:0,bc:0,sa:0,saAdopted:0,save:0,unread:0,expired:0});
    ...
    root.innerHTML=rows.map(r=>`<div class="source-audit-row" data-source-audit="${escV9(r.source)}">
      ...
      <div class="source-audit-numbers"><span>来源跳过 <b>${r.skip}</b>/${r.total} (${fmtPct(r.skipRate)})</span><span>显 ${r.explicitSkip}</span><span>隐 ${r.implicitSkip}</span><span>B/C ${r.bc}</span><span>S/A采纳 ${r.saAdopted}/${r.sa}</span><span>过期 ${r.expired}</span><span>保存 ${r.save}</span><span>待看 ${r.unread}</span></div>
    </div>`).join('');
  ...
  async function copyStats(){
    ...
    const lines=[`Weekly来源统计｜来源评估跳过 ${tot.skip}（显性 ${tot.explicit} + 未标注 ${tot.implicit}）`,'媒体\t来源跳过/总数\t跳过率\t显性\t隐性\tB/C\tS/A采纳\t过期\t保存\t待看\t建议'];
    for(const r of rows)lines.push(`${r.source}\t${r.skip}/${r.total}\t${fmtPct(r.skipRate)}\t${r.explicitSkip}\t${r.implicitSkip}\t${r.bc}\t${r.saAdopted}/${r.sa}\t${r.expired}\t${r.save}\t${r.unread}\t${r.action}`);
  ...
  // For the unread view, 7-day expiry is a queue rule, not a feedback mutation.
  // B/C and expired S/A disappear from attention without being converted into human skip records.
  ...
  updateProgressTabs=function(){
    ...
    const note=document.getElementById('autoArchiveNote');if(note)note.textContent=`B/C 自动归档 ${autoArchived} 篇；S/A 7天未处理归档 ${expired} 篇（都不算强负反馈）`;
  };
  ...
  window.weeklySourceAuditV9={sourceStats,render:renderSourceAudit,copy:copyStats,isRecommendedUnread,isExpiredUnlabeled};
})();
