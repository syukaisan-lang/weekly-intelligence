(() => {
  ...
  function relationScore(article,k){
    ...
    const ktext=textOfKnowledge(k);const ac=new Set(concepts(atext)),kc=new Set(concepts(ktext));let score=0;
    ac.forEach(x=>{if(kc.has(x))score+=2.2;});
    const aw=new Set(words(article.title||'')),kw=new Set(words(k.title||''));aw.forEach(x=>{if(kw.has(x))score+=x.length>=5?1.25:.7;});
    const broad=new Set(words(atext)),kbroad=new Set(words(ktext));let shared=0;broad.forEach(x=>{if(kbroad.has(x))shared++;});score+=Math.min(shared,5)*.22;
    if(article.url&&k.url&&article.url===k.url)score=-99;return score;
  }
  ...
  function inject(){
    if(!knowledge||typeof data==='undefined'||!Array.isArray(data.articles))return;
    document.querySelectorAll('#articleList .article').forEach(card=>{
      ...
      const href=link.getAttribute('href'),title=link.textContent.trim();const article=(data.articles||[]).find(a=>a.url===href||a.title===title);if(!article)return;
      const rows=related(article);if(!rows.length)return;
  ...
  const oldRender=renderArticles;
  renderArticles=function(){oldRender();setTimeout(inject,0);};
