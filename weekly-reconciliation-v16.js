// v16.2: auditable weekly totals aligned with S/A-only reading queue semantics.
(() => {
  ...
  function bucket(a){
    ...
    return ['S','A'].includes(g(a))?'queue':'c';
  }
  ...
  function render(){
    ...
    root.innerHTML=`<div class="weekly-recon-title"><b>本周 ${r.total}</b><span>同一口径：以下 5 项互斥，相加 = 本周总数</span></div><div class="weekly-recon-grid"><button data-recon="queue">待处理 S/A <b>${r.queue}</b></button><button data-recon="later">稍后看 <b>${r.later}</b></button><button data-recon="done">已处理/保存 <b>${r.done}</b></button><button data-recon="skip">已跳过 <b>${r.skip}</b></button><button data-recon="c">B/C隐藏 <b>${r.c}</b></button></div><div class="weekly-recon-check">${r.queue}+${r.later}+${r.done}+${r.skip}+${r.c} = ${r.total}</div>`;
    ...
    if(unread){unread.childNodes[0].nodeValue='当前队列 ';unread.title='跨周的 S/A 待处理队列；B/C 不进入优先阅读，正面反馈仍保留未读，负面反馈视为已处理';}
  }
