...
(() => {
  ...
  function facetDelta(rows){
    // Trend inference is allowed only from medium/high-confidence evidence dates.
    const dated=rows.filter(x=>x.tm.effective&&x.tm.confidence!=='low');if(dated.length<4)return[];const sorted=[...dated].sort((a,b)=>a.tm.effective.localeCompare(b.tm.effective)),n=Math.max(2,Math.floor(sorted.length/3)),early=sorted.slice(0,n),late=sorted.slice(-n),out=[];
  ...
  function render(query,rows){
    ...
    const strong=rows.filter(x=>x.tm.confidence!=='low'),basisRows=strong.length>=2?strong:rows,years=[...new Set(basisRows.map(x=>String(x.tm.effective||'').slice(0,4)).filter(Boolean))],facets=facetDelta(strong),first=years[0]||'—',last=years.at(-1)||'—',enough=strong.length>=4;
    status.textContent=`“${query}” · 找到 ${rows.length} 条语义相关证据，${strong.length} 条具备中/高时间置信度。`;
    const conclusion=enough?(facets.length?`<div><b>初步时间结论</b><div class="temporal-facets">${facets.map(x=>`<span class="temporal-facet">${esc(x)}</span>`).join('')}</div><p class="muted small temporal-note">变化信号只由中/高置信的调查期或发布日期证据计算。↑/↓ 表示相关证据中该主题出现频率的方向性变化，不等于同口径统计指标的同幅度变化。</p></div>`:`<div class="muted small temporal-note"><b>初步时间结论：</b>已有足够时间证据，但当前语义结果没有检测到明显的主题方向变化；请结合下方具体调查口径判断。</div>`):`<div class="muted small temporal-note"><b>证据不足：</b>中/高置信时间证据少于4条，暂不输出趋势方向。低置信“收录日”资料只作为补充阅读，不参与变化判断。</div>`;
    sum.innerHTML=`<div class="temporal-summary-grid"><div class="temporal-stat"><span class="muted small">有效证据时间</span><b>${esc(first)} → ${esc(last)}</b></div><div class="temporal-stat"><span class="muted small">有效年份</span><b>${years.length}</b></div><div class="temporal-stat"><span class="muted small">强时间证据</span><b>${strong.length}/${rows.length}</b></div></div>${conclusion}`;
